...
// أضف هذه الدالة في ملف app.js
async function autoUnsuspendFarms() {
  try {
    const now = new Date();
    // 1. جلب كل المستخدمين الذين لديهم اشتراك ساري المفعول حالياً
    const activeUsers = await User.find({ 
      subscriptionExpiresAt: { $gt: now } 
    }).select('_id subscriptionTier');

    for (let user of activeUsers) {
      // 2. إلغاء التعليق عن الأراضي التي عُلقت فقط بسبب "الحد" (limit)
      // هذا يضمن عدم تفعيل الأراضي التي حذفها الأدمن يدوياً لأسباب أخرى
      await Farm.updateMany(
        { 
          owner: user._id, 
          isSuspended: true, 
          suspendedReason: 'limit' 
        },
        { 
          $set: { isSuspended: false, suspendedReason: '' } 
        }
    ...
    console.log('✅ Auto-unsuspend job completed');
  } catch (err) {
    console.error('❌ Error in autoUnsuspendFarms:', err);
  }
}
  setInterval(async () => {
    try {
