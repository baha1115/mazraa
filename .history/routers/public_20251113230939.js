...
router.get('/api/farms/sale', async (req, res) => {
  try {
    ...
    const rows = await Farm.aggregate([
      { $match: { kind:   { $regex: /^sale$/i },                     // ← بدل 'sale'
      status: { $regex: /^approved$/i }, isSuspended: { $ne: true }, deletedAt: null } },
      {
        $lookup: {
          from: 'users',
          localField: 'owner',
...
// GET /api/farms/rent?vipOnly=1&limit=60
router.get('/api/farms/rent', async (req, res) => {
  try {
    const vipOnly = String(req.query.vipOnly || '') === '1';
    const limit   = Math.min(parseInt(req.query.limit || '60', 10), 96);

    // لو عندك denormalized ownerTier داخل Farm يكون أسرع بكثير
    const match = {
      kind:   { $regex: /^rent$/i },
      status: { $regex: /^approved$/i },
      isSuspended: { $ne: true },
      $or: [{ deletedAt: null }, { deletedAt: { $exists: false } }],
      ...(vipOnly ? { ownerTier: 'VIP' } : {})
    };

    const rows = await Farm.find(match)
      .select('title area city size price currency photos videoUrl views ownerTier kind createdAt')
      .sort({ createdAt: -1 })
      .limit(limit)
      .lean();
...
function smallImg(u=''){
  return /^https?:\/\/res\.cloudinary\.com\//.test(u)
    ...
    // أعد صورة الغلاف فقط لتخفيف الـJSON
...
router.get('/farm/:id', async (req, res) => {
  ...
  try {
    ...
    const farm = await withTimeout(
      Farm.findByIdAndUpdate(id, { $inc: { views: 1 } }, { new: true }).lean(),
      8000
    );
