...
// حدود آمنة أكثر
const MAX_FILE_BYTES   = 2 * 1024 * 1024;  // 2MB للصورة الواحدة
const MAX_TOTAL_BYTES  = 5 * 1024 * 1024;  // 5MB للطلب كامل (هنستخدمه بعد شوي)

const uploadMem = multer({
  storage: multer.memoryStorage(),
  limits: {
    fileSize: MAX_FILE_BYTES, // كل ملف بحد أقصى 2MB
    files: 10                 // أقصى عدد ملفات في الطلب
  }
});
...
// تقدير حجم dataURL بالبايت بدون عمل Buffer فعلي كبير
...
if (req.body && typeof req.body.phone !== 'undefined') {
  ...
  req.body.phone = n;              // اكتب المطبع أو '' لو فشل // يجب أن تظهر في الطرفية
}
...
router.post(
  ...
  async (req,res)=>{
    try{
      ...
      // ===== فحص إجمالي حجم الصور لهذا الطلب (ملفات + DataURL) =====
      let totalBytes = 0;

      // 1) ملفات avatar + photos من multer
      ...
      if (totalBytes > MAX_TOTAL_BYTES) {
        return res.status(400).json({
          ok:false,
          msg:'إجمالي حجم الصور كبير جداً (الحد الأقصى 5MB لكل طلب).'
        });
      }
      // ===== نهاية فحص الحجم =====
...
if (req.files?.avatar?.[0]?.buffer) {
  ...
  // ✅ لو جاية Base64 ارفعها إلى Cloudinary بدل تخزين data:
  if (v.startsWith('data:image/')) {
    const buf = dataURLtoBuffer(v);               // عندك الدالة فوق بالفعل
    const up  = await uploadBufferToCloudinary(
      buf,
      { folder:'contractors', publicId: 'avatar_'+Date.now() }
    );
    ...
    // رابط جاهز http(s) أو /uploads/ قديم
    avatar = v;
  }
}
...
router.post('/contractor/subscriptions', requireAuthApi, async (req,res)=>{
  try{
