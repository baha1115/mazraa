...
async function applyPlanLimitsForUser(userId, tier) {
 ...
 const farms = await Farm.find({ 
  owner: userId, 
  deletedAt: null,
  status: { $in: ['approved', 'Approved'] }, // ← أضف هذا السطر
}).sort({ createdAt: -1 });
...
router.get('/debug/farms/vip-approved', async (req, res) => {
  try {
    ...
    const rows = await Farm.aggregate([
      ...
      { $match: { ownerTier: { $regex: /^vip$/i } } },
      { $limit: limit },
      { $project: { title: 1, price: 1, city: 1, createdAt: 1, owner: 1, ownerTier: 1 } }
    ]);

    return res.json({ ok: true, count: rows.length, rows });
  } catch (e) {
    console.error(e);
    return res.status(500).json({ ok: false, msg: 'vip_debug_failed' });
  }
});
router.get('/debug/users/:userId/farms-state', async (req, res) => {
  try {
    const { userId } = req.params;

    const user = await User.findById(userId)
      .select('_id name email subscriptionTier subscriptionExpiresAt subscriptionUntil')
      .lean();

    if (!user) return res.status(404).json({ ok: false, msg: 'user_not_found' });

    const farms = await Farm.find({ owner: userId })
      .select('_id kind status isSuspended suspendedReason deletedAt createdAt')
      .lean();

    const approved = farms.filter(f => String(f.status).toLowerCase() === 'approved');
    const approvedSuspended = approved.filter(f => f.isSuspended === true);
    const approvedDeleted = approved.filter(f => f.deletedAt);
    ...
    return res.json({
      ...
      counts: {
        ...
        approvedSuspended: approvedSuspended.length,
        approvedDeleted: approvedDeleted.length,
      },
      samples: {
        approvedSuspended: approvedSuspended.slice(0, 10),
        approvedDeleted: approvedDeleted.slice(0, 10),
      }
    });
  } catch (e) {
    console.error(e);
    return res.status(500).json({ ok: false, msg: 'debug_failed' });
  }
});
