...
function normalizeSyWhatsapp(raw){
  ...
  if (!hadPlus && digits.startsWith('0')) digits = digits.slice(1);

  if (digits.startsWith('963')) return '+' + digits;
  if (!hadPlus && digits.startsWith('9')) return '+963' + digits;
...
// تقدير حجم dataURL بالبايت (بدون فكها فعلياً)
function approxDataUrlBytes(dataUrl){
  if (typeof dataUrl !== 'string') return 0;

  // نفصل الهيدر عن البايس64
  const commaIndex = dataUrl.indexOf(',');
  if (commaIndex === -1) return 0;

  const b64 = dataUrl.slice(commaIndex + 1).trim();

  // نحسب حجم البايتات من طول البايس64 (مع التعامل مع padding)
  let padding = 0;
 ...
 */
async function buildPhotosArrayCloud(req, { folder='farms', watermark=false, watermarkText='مزرعة' } = {}){
  const out = [];

  const files = Array.isArray(req.files) ? req.files : (req.files?.photos || []);
  for (const f of files){
    if (f?.buffer){
      const r = await uploadBufferToCloudinary(f.buffer, { folder, watermark, watermarkText });
  ...
  if (Array.isArray(bodyPhotos)){
    for (const item of bodyPhotos){
      if (typeof item === 'string' && item.startsWith('data:image/')){
        ...
        const r = await uploadBufferToCloudinary(buf, { folder, watermark, watermarkText });
        if (r?.secure_url) out.push(r.secure_url);
      } else if (typeof item === 'string' && /^https?:\/\//.test(item)){
        out.push(item); // روابط جاهزة تبقى كما هي
      }
    }
  }
...
router.post('/owner/lands', requireAuth, uploadMem.array('photos', 12), async (req, res) => {
  try {
    ...
    // ===== فحص حصة الخطة (plan quota) =====
    const userId = req.session.user._id;
    const user   = await User.findById(userId).lean();
    const plan   = (user?.subscriptionTier || 'Basic');

    const used   = await Farm.countDocuments({ owner: userId }).maxTimeMS(4000);
    const limit  = planLimit(plan);
    const unlimited = (limit === Infinity);
    const leftBefore = unlimited ? null : Math.max(0, limit - used);

    // لو الخطة ليست غير محدودة ولا يوجد أي إعلان متبقّي
    if (!unlimited && leftBefore <= 0) {
      return res.status(400).json({
        ok: false,
        msg: 'بلغت حد النشر المسموح به في خطتك. قم بحذف إعلان قديم أو ترقية الاشتراك.',
        data: { plan, limit, used, left: leftBefore }
      });
    }
    // ===== نهاية فحص الحصة =====
  ...
  const photos = await buildPhotosArrayCloud(req, { folder:'farms', watermark:true, watermarkText:'مزرعتي' });
    ...
    const ownerInfo = {
      ...
      whatsapp: normalizeSyWhatsapp(value.ownerWhatsapp),
    };
    ...
    // نحدّث قيم الحصة بعد إنشاء الإعلان (واحد جديد أُضيف)
    const usedAfter = used + 1;
    const leftAfter = unlimited ? null : Math.max(0, limit - usedAfter);

    return res.json({
      ok:true,
      msg:'تم إرسال إعلانك للمراجعة',
      data:{
        id: doc._id,
        plan,
...
router.get('/owner/lands/quota', requireAuth, async (req, res) => {
  try {
    ...
    const unlimited = (limit === Infinity);
    const safeLimit = unlimited ? null : limit;
    const left      = unlimited ? null : Math.max(0, safeLimit - used);

    return res.json({
      ok: true,
      data: {
        plan,
        limit: safeLimit,   // يبقى null في حالة VIP (نفس ما كان يحدث قبل)، لكن أوضح
        used,
        left,
        unlimited           // معلومة إضافية لا تكسر أي كود قديم
      }
    });
  } catch (e) {
    console.error(e);
   ...
   const photos = await buildPhotosArrayCloud(req, { folder:'farms', watermark:true, watermarkText:'مزرعتي' });
    ...
    : null,
      ...
      ownerInfo: {
        ...
        whatsapp: normalizeSyWhatsapp(b.ownerWhatsapp),
