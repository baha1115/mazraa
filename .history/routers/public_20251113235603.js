  ...
  try {
    ...
    const pipeline = [
      ...
      { $addFields: {
          ownerTier: {
            $let: {
              vars: { t: { $ifNull: [ { $arrayElemAt: ['$u.subscriptionTier', 0] }, 'Basic' ] } },
              in: {
                $switch: {
                  branches: [
                    { case: { $eq: [ { $toLower: '$$t' }, 'vip' ] },     then: 'VIP' },
                    { case: { $eq: [ { $toLower: '$$t' }, 'premium' ] }, then: 'Premium' },
                  ],
                  default: 'Basic'
                }
              }
      ...
      { $project: {
          u: 0,
          title: 1, area:1, city:1, size:1, price:1, currency:1,
    ...
    let rows = await Farm.aggregate(pipeline);

    // تصغير Cloudinary (اختياري لكن مفيد)
